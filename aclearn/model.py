...
from aclearn.dataset import ALDataset
...
class ALModel():
    def __init__(self,query_strategy,logger,is_oracle=False,nb_init_label_per_class=1,dataset_name=''):
        """
        query_strategy (func):
        is_oracle (bool):
        nb_init_label_per_class (int):
        """
        ...
        self.dataset = ALDataset(nb_init_label_per_class, dataset_name)
    ...
    def evaluate_max(self):
        #Compute the maximum accuracy to be able to compare performances
    ...
    def active_learning_procedure(self, n_queries=10, query_size=10,train_acc=False):
        ...
        for index in range(n_queries):
            ...
            if train_acc:
                self.logger.debug('(query {n}) Train acc: \t{tr_acc:0.4f}  |  Test acc: \t{tst_acc:0.4f}'.format(n=index+1,
                    tr_acc=self.acc_train_history[index], tst_acc=self.acc_history[index]))
            else:
                self.logger.debug('(query {n}) Test acc: \t{acc:0.4f}'.format(n=index+1, acc=self.acc_history[index]))
