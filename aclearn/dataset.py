...
class AcLearnDataset:
    def __init__(
        self, data_path, label_path=None, data_path_unlab=None, size_init_per_class=1
    ):
        """_summary_

        Args:
            data_path (_type_): _description_
            label_path (_type_, optional): _description_. Defaults to None.
            data_path_unlab (_type_, optional): _description_. Defaults to None.
            size_init_per_class (int, optional): _description_. Defaults to 1.
        """
    ...
    def read_famous_dataset(self, demo_name):
        ...
        Args:
            demo_name (_type_): _description_
        """
    ...
    def read_dataset(self, data_path, label_path, data_path_unlab, extension="npy"):
        ...
        Args:
            ...
            data_path_unlab (_type_): _description_
            extension (str, optional): _description_. Defaults to "npy".
        """
    ...
    def preprocessing(self):
        ...
        self.X_train = self.X_train.reshape(
            self.X_train.shape[0], 1, self.image_size[0], self.image_size[1]
        )
        self.X_test = self.X_test.reshape(
            self.X_test.shape[0], 1, self.image_size[0], self.image_size[1]
        )

        self.X_pool = self.X_pool.reshape(
            self.X_pool.shape[0], 1, self.image_size[0], self.image_size[1]
        )
        self.X_init = self.X_init.reshape(
            self.X_init.shape[0], 1, self.image_size[0], self.image_size[1]
        )
    ...
    def init_label_pool(self, nb_init_label_per_class):
        ...
        Args:
            nb_init_label_per_class (_type_): _description_
        """
        ...
        for i in range(self.nb_class):
            idx = np.random.choice(
                np.where(self.y_train == i)[0],
                size=nb_init_label_per_class,
                replace=False,
            )
