...
def max_entropy(learner, X, query_size=1, T=100, max_sample=2000):
    ...
    with torch.no_grad():
        outputs = np.stack(
            [
                torch.softmax(
                    learner.estimator.forward(X[random_subset], training=True), dim=-1
                )
                .cpu()
                .numpy()
                for t in range(100)
...
def bald(learner, X, query_size=1, T=100, max_sample=2000):
    ...
    E_H = -np.mean(
        np.sum(outputs * np.log(outputs + 1e-10), axis=-1), axis=0
    )  # [batch size]
    acquisition = H - E_H
    idx = (-acquisition).argsort()[:query_size]
    query_idx = random_subset[idx]
...
def variation_ratio(learner, X, n_instances=1, T=100, max_sample=2000):
    ...
    acquisition = 1 - np.amax(
        pc, axis=-1
    )  # TODO complete with variation ratio formula and pc
