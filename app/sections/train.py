...
def train_window():
    ...
    if st.session_state["oracle"] == "user":
        st.markdown(
            f"## Labeling: <font color='gray'>{st.session_state.task}",
    ...
    if retrain:
        state = st.empty()
        msg = st.empty()
        progress_bar = [st.progress(0.0), 0.0, 1.0]
        ...
        for i in range(1, st.session_state.n_models + 1):
            if st.session_state[f"n_samp_mod_{i}"] > 1:
                ...
                for j in range(st.session_state[f"n_samp_mod_{i}"]):
                    progress_bar[1] = min + (
                        j / st.session_state[f"n_samp_mod_{i}"]
                    ) * (max - min)
                    progress_bar[2] = min + (j + 1) / st.session_state[
                        f"n_samp_mod_{i}"
                    ] * (max - min)

                    state.markdown(
                        f"<center>model {i}/{st.session_state.n_models}<br>sample [{j+1}/{st.session_state[f'n_samp_mod_{i}']}]</center>",
                        unsafe_allow_html=True,
                    )
                    training(
                        f"model_{i}.{j}", msg, progress_bar, tsne=(j == 0), pca=True
                    )
                ...
                state.markdown(
                    f"<center>model {i}/{st.session_state.n_models}</center>",
                    unsafe_allow_html=True,
                )
...
def training(model_name, msg, progress_bar, tsne=True, pca=True):
    ...
    st.session_state[model_name].active_learning_procedure(
        n_queries=st.session_state["n_epochs"],
    ...
    if tsne:
        msg.markdown(
            f"<center><font color='gray'>compute t-SNE</center>", unsafe_allow_html=True
        )
    ...
    if pca:
        msg.markdown(
            f"<center><font color='gray'>compute PCA</center>", unsafe_allow_html=True
        )
        st.session_state[model_name].compute_pca()
        print(f"$({st.session_state[model_name].model_id}) pca computed")

    msg.markdown(
        f"<center><font color='gray'>compute figure</center>", unsafe_allow_html=True
    )
...
def training_parameters_section():
    ...
    if st.session_state["oracle"] == "computer":
        with cols[4]:
            st.session_state["n_epochs"] = st.number_input(
                "number of epochs", 1, 500, 8, step=1
            )
